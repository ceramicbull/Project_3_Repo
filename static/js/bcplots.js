const map = L.map('map').setView([37.8, -96], 4);
	...
	info.update = function (props) {
		const contents = props ? `<b>${props.proper_name}</b><br />${props.sightings} sightings reported` : 'Hover over a state';
		this._div.innerHTML = `<h4># of Sightings</h4>${contents}`;
	};
	...
	d3.json(geoUrl).then(function(data){
		...
		legend.onAdd = function (map) {
			...
			for (let i = 0; i < grades.length; i++) {
				...
				labels.push(`<i style="background:${getColorSight(from + 1)}"></i> ${from}${to ? `&ndash;${to}` : '+'}`);
			}
